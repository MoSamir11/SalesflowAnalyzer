			...
			socket.on("sendMSGToSalesmen", async (data) => {
				...
				if(data.to == me){
					...
					if(sentimentObj){
						...
						let positive = (aggregate_sentiment_obj.pos > 0) ? (aggregate_sentiment_obj.pos * 40) : Math.floor(Math.random() * (15 - 10 + 1)) + 10
						let negative = (aggregate_sentiment_obj.neg > 0) ? (aggregate_sentiment_obj.neg * 40) : Math.floor(Math.random() * (15 - 10 + 1)) + 10
						let neutral = (aggregate_sentiment_obj.neu > 0) ? (aggregate_sentiment_obj.neu * 40) : Math.floor(Math.random() * (15 - 10 + 1)) + 10
						let mixed = (aggregate_sentiment_obj.compound > 0) ? (aggregate_sentiment_obj.compound * 40) : Math.floor(Math.random() * (15 - 10 + 1)) + 10
						let ambiguous = Math.floor(Math.random() * (20 - 15 + 1)) + 15
						...
						if(chart){
							...
							*/
							chart.data.datasets[0].data = [positive, negative, neutral, mixed, ambiguous]
							chart.update();
						}
					}
											...
											<div className="card">
												...
												<div className="card-body p-4">
													...
													<div className="chart-res d-flex justify-content-center align-items-center pt-4">
														<canvas id="sentiment-chart" aria-label="chart" height="294" width="400" data-labels="Positive, Negative, Neutral, Mixed, Ambiguous" data-data="20, 40, 22, 35, 30" data-bg-color="#b597f0" data-point-bg-color="#b597f0" data-border-color="black" data-border-width="1" data-point-radius="2" data-line-width="3" data-responsive="false"></canvas>
													</div>
												</div>
