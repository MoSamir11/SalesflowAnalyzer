...
import { ExpressionChart, SentimentGraph } from "./expression-chart.js";
import { ChatBox, PitchInfo } from "./chatbox.js";
...
function Dashboard() {
  ...
  const [sentimentChartState, setSentimentChartState] = useState({
    options: {
  ...
  const [expressionChartState, setExpressionChartState] = useState({
    options: {
      colors: ["#D273F2", "#6f42c1"],
      chart: {
        id: "basic-bar",
    ...
    if (person == "SalesPerson") {
      ...
      socketRef.current.on("sendMSGToSalesmen", async (data) => {
        ...
        if (data.userType == "Client") {
          ...
          sentiment_array.push(userSentimentScore);
          let speechTime = data.time.substring(0,data.time.lastIndexOf(":")+1,data.time)
          sentiment_time.push(speechTime);
      setSentimentChartState((prevState) => ({
        ...prevState,
        options: {
          ...prevState.options,
          xaxis: {
            ...prevState.options.xaxis,
            categories: sentiment_time,
          },
        },
        series: [
          {
      ...
      if (clientVideo.current) {
        ...
        if (detections) {
          if (detections.length > 0) {
            ...
            setExpressionChartState((prevState) => ({
              ...prevState,
              options: {
                ...prevState.options,
                xaxis: {
                            ...
                            <div className="accordian">
                              <div className="">
                                ...
                                <div className={selected ? "content show" : "content" } style={{ display: "flex" }}>
                                  <ExpressionChart title="Expression Analysis"/>

                                  <SentimentGraph title="Sentiment Analysis"/>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <ChartComp sentimentChart={sentimentChartState} expressionChart={expressionChartState} selected={selected} />
                        ...
                        <div className="modal-content">
                          ...
                          <div className="modal-body px-5" id="tabbbs">
                            <div className="row px-1">
                              ...
                              <ChatBox chatBoxData={chatBoxData} getClassName={getClassName} />
                              </div>
                            </div>
                          </div>
                        </div>
